...
sh ./distclean.sh
cmake . $BUILD_OPTIONS && make install && make -j6 package || exit 1
...
if [ $RUN_TESTS = "true" ]; then
    ...
    if [ $? == 1 ]; then
        echo "10.1.1.242  theater oracledb dbhost_oracle dbhost_mssql dbhost_pg dbhost_mysql smtp_host redis_server" >> /etc/hosts
    fi
