/***************************************************************************
    ...
    copyright            : (C) 2004-2012 by Alexey S.Parshin
...
public:
   CFileListTreeHeader(CTreeItem *item);
...
public:
   static CLayoutClient* itemCreator(CTreeItem *item);
public:
   CFileListTreeFolder(CTreeItem *item);
...
public:
   CFileListTreeItem(CTreeItem *item);
...
public:
   CFileManager(const char* label="", int layoutSize=50, CLayoutAlign layoutAlign=SP_ALIGN_CLIENT) : CGroup(label, layoutSize, layoutAlign) {
      layoutSpacing(0);
...
void CFileManagerTree::refreshData() {
   ...
   CQuery files(&m_db, "SELECT title, filename, filesize, fileuint32_transferred, publisher, publish_date FROM content WHERE channel=:channel ORDER BY 1");
   folders.open();
   while (!folders.eof()) {
   ...
   files.sql("SELECT title, filename, filesize, fileuint32_transferred, publisher, publish_date FROM content WHERE channel is NULL ORDER BY 1");
   files.open();
   int counter = 0;
   while (!files.eof()) {
      int64_t filesize = files["filesize"];
