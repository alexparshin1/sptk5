...
uint32_t CGIApplication::objectCount;
...
void CGIApplication::sendHeader() {
#ifdef _WIN32
...
bool CGIApplication::begin(HTML_TAGS tag, const char *params) {
...
bool CGIApplication::end(HTML_TAGS tag) {
    bool rc = false;
...
bool CGIApplication::insertCRLF() {
    return printString("\n\r");
}
//---------------------------------------------------------------------------

bool CGIApplication::printString(const char *s) {
    bool rc = false;
    if (m_showPreview && m_htmlPrint) {
        fputs(s, m_htmlPrint);
        rc = true;
...
void CGIApplication::startTable(const char *params) {
    if (insertCRLF())
        begin(TABLE_TAG, params);
}

void CGIApplication::endTable() {
    if (end(TABLE_TAG))
        insertCRLF();
}

void CGIApplication::startTableRow(const char *params) {
    if (insertCRLF())
        begin(TABLEROW_TAG, params);
}

void CGIApplication::endTableRow() {
    if (end(TABLEROW_TAG))
        insertCRLF();
}

void CGIApplication::tableHeaderCell(const char *cellData, const char *params) {
...
void CGIApplication::tableCell(const char *cellData, const char *params) {
...
void CGIApplication::printParameters() {
    HttpParams::iterator itor = contentData.begin();
...
void CGIApplication::beginPage(const char *title, const char *bodyParams) {
    if (begin(HTML_TAG)) {
        insertCRLF();
        printString("<HEAD>");
        begin(TITLE_TAG);
...
void CGIApplication::addTextInput(const char *name, const char *value, const char *attr) {
    if (m_htmlPrint)
...
void CGIApplication::addHiddenInput(const char *name, const char *value, const char *attr) {
    if (m_htmlPrint)
...
FILE *CGIApplication::createHtmlCopyFile(const char *fname) {
    if (!m_htmlFile)
        m_htmlFile = fopen(fname, "wt");
    return m_htmlFile;
}

void CGIApplication::closeHtmlCopyFile() {
    if (m_htmlFile)
        fclose(m_htmlFile);
    m_htmlFile = 0;
}

void CGIApplication::showPreviewMode(bool sp) {
    m_showPreview = sp;
}

bool CGIApplication::showPreviewMode() {
