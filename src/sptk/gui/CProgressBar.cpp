...
public:	
   ...
   bool m_showText;
public:
   CProgressBox(int x,int y,int w,int h) : Fl_Box(x,y,w,h) { 
      m_value = 0;
      m_minValue = 0;
      m_maxValue = 100;
      m_showText = true;
...
void CProgressBox::draw() {
   ...
   if (!CThemes::drawProgressBar(x(),y(),w(),percent)) {
      fl_draw_box(box(),x(),y(),w(),h(),parent()->color());
      int width = int((w() - Fl::box_dw(box())) * percent / 100 + 0.5);
      fl_draw_box(FL_THIN_UP_BOX,x() + Fl::box_dx(box()),y() + Fl::box_dy(box()), width, h() - Fl::box_dh(box()), FL_BLUE);
   }
