...
CSpellOption::CSpellOption(string name,string value) { 
   m_name = name; 
   m_value = value; 
}
...
CSpellOption& CSpellOption::operator = (string value) {
   m_value = value; 
   return *this;  
}
...
void CSpellChecker::cb_suggest(Fl_Widget *lv,void *) {
   CListView *listView = (CListView *)lv;
...
CSpellOption& CSpellChecker::operator [] (string optionName) {
   CSpellOptions::iterator itor = CSpellOptions::find(optionName);
