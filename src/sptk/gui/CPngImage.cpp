...
static Fl_RGB_Image* subRGBImage(Fl_RGB_Image* image,unsigned offsetX,unsigned offsetY,unsigned width,unsigned height) {
...
void CPngImage::cutStretchDraw(CPngImage* sourceImage,int srcX,int srcY,int srcW,int srcH,int destX,int destY,int destW,int destH) {
    if (destW <= 0 || destH <= 0) return;
    Fl_RGB_Image* img = subRGBImage(sourceImage,srcX,srcY,srcW,srcH);
    Fl_RGB_Image* stretched = (Fl_RGB_Image*)img->copy(destW,destH);
    stretched->draw(destX,destY);
    delete img;
    delete stretched;
}

void CPngImage::cutTileDraw(CPngImage* sourceImage,int srcX,int srcY,int srcW,int srcH,int destX,int destY,int destW,int destH) {
    if (destW <= 0 || destH <= 0) return;
    Fl_RGB_Image* img = subRGBImage(sourceImage,srcX,srcY,srcW,srcH);
...
void CPngImage::drawResized(int xx,int yy,int ww,int hh,int cornerWidth,CPatternDrawMode drawMode,bool drawBackground) {
    ...
    resizedDraw(this,cornerSizeX,0,w()-cornerSizeX*2,cornerSizeY,xx+cornerSizeX,yy,ww-cornerSizeX*2,cornerSizeY);
    resizedDraw(this,cornerSizeX,h()-cornerSizeY,w()-cornerSizeX*2,cornerSizeY,xx+cornerSizeX,yy+hh-cornerSizeY,ww-cornerSizeX*2,cornerSizeY);
    resizedDraw(this,0,cornerSizeY,cornerSizeX,h()-cornerSizeY*2,xx,yy+cornerSizeY,cornerSizeX,hh-cornerSizeY*2);
    resizedDraw(this,w()-cornerSizeX,cornerSizeY,cornerSizeX,h()-cornerSizeY*2,xx+ww-cornerSizeX,yy+cornerSizeY,cornerSizeX,hh-cornerSizeY*2);
    if (drawBackground)
        resizedDraw(this,cornerSizeX,cornerSizeY,w()-cornerSizeX*2,h()-cornerSizeY*2,xx+cornerSizeX,yy+cornerSizeY,ww-cornerSizeX*2,hh-cornerSizeY*2);
}
...
void CPngImage::drawResized(int xx,int yy,int ww,int hh,int border[],CPatternDrawMode drawMode,bool drawBackground) {
    ...
    draw(xx,yy+hh-border[BORDER_BOTTOM],border[BORDER_LEFT],border[BORDER_BOTTOM],0,imageHeight-border[BORDER_BOTTOM]);
    draw(xx+ww-border[BORDER_RIGHT],yy,border[BORDER_LEFT],border[BORDER_TOP],imageWidth-border[BORDER_RIGHT],0);
    draw(xx+ww-border[BORDER_RIGHT],yy+hh-border[BORDER_BOTTOM],border[BORDER_LEFT],border[BORDER_BOTTOM],imageWidth-border[BORDER_RIGHT],imageHeight-border[BORDER_BOTTOM]);
    ...
    /// Draw frame parts 
    if (xSideSpace > 0) {
        resizedDraw(this,border[BORDER_LEFT],0,xSideSpace,border[BORDER_TOP],xx+border[BORDER_LEFT],yy,ww-xBorderSpace,border[BORDER_TOP]);
        resizedDraw(this,border[BORDER_LEFT],h()-border[BORDER_BOTTOM],xSideSpace,border[BORDER_BOTTOM],xx+border[BORDER_LEFT],yy+hh-border[BORDER_BOTTOM],ww-xBorderSpace,border[BORDER_BOTTOM]);
    }
    if (ySideSpace > 0) {
        resizedDraw(this,0,border[BORDER_TOP],border[BORDER_LEFT],ySideSpace,xx,yy+border[BORDER_TOP],border[BORDER_LEFT],hh-yBorderSpace);
        resizedDraw(this,w()-border[BORDER_RIGHT],border[BORDER_TOP],border[BORDER_RIGHT],ySideSpace,xx+ww-border[BORDER_RIGHT],yy+border[BORDER_TOP],border[BORDER_RIGHT],hh-yBorderSpace);
    }
    if (drawBackground && xSideSpace > 0 && ySideSpace > 0)
        resizedDraw(this,border[BORDER_LEFT],border[BORDER_TOP],xSideSpace,ySideSpace,xx+border[BORDER_LEFT],yy+border[BORDER_TOP],ww-xBorderSpace,hh-yBorderSpace);
}
