...
void CCalendar::cbDayButtonClicked(Fl_Widget *button,void *param) {
   ...
   CCalendar  *calendar = dynamic_cast<CCalendar *>(buttonBox->parent());
   if (!calendar) return;
...
void CCalendar::dayButtonClicked(uint32_t day) {
   ...
   CPopupCalendar *w = dynamic_cast<CPopupCalendar *>(btn->window());
   if (w) {
      w->clicked();
      w->hide();
   }
...
void CCalendar::resize(int xx,int yy,int ww,int hh) {
   ...
   CPopupCalendar *pcw = dynamic_cast<CPopupCalendar *>(btn->window());
   if (pcw) {
