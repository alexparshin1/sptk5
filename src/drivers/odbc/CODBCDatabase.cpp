...
string CODBCDatabase::queryError(SQLHSTMT stmt) const {
    ...
    *errorState = 0;
    ...
    int rc = SQLError(SQL_NULL_HENV, (SQLHDBC)handle(), stmt, errorState, &nativeError, errorDescription, sizeof(errorDescription), &pcnmsg);
    if (rc != SQL_SUCCESS) {
        rc = SQLError(SQL_NULL_HENV, (SQLHDBC)handle(), 0L, errorState, &nativeError, errorDescription, sizeof(errorDescription), &pcnmsg);
        if (rc != SQL_SUCCESS)
            if (!*errorDescription)
...
void CODBCDatabase::queryExecute(CQuery *query) {
    ...
    if (rc != SQL_SUCCESS) {
        ...
        int rc = SQLGetDiagField(
                SQL_HANDLE_STMT, query->statement(), 1, SQL_DIAG_NUMBER,
                &recordCount, sizeof(recordCount), &textLength);
        if (!successful(rc))
        ...
        for (SQLSMALLINT recordNumber = 1; recordNumber <= recordCount; recordNumber++) {
            rc = SQLGetDiagRec(
                    SQL_HANDLE_STMT, query->statement(),
                    recordNumber, state, &nativeError,
                    text, sizeof(text), &textLength);
            if (!successful(rc))
                break;
        ...
        for (unsigned j = 0; j < param->bindCount(); j++) {
            ...
            switch (ptype) {
                ...
                case VAR_BUFFER: {
                    ...
                    cblen = len;
                    rc = SQLBindParameter((SQLHSTMT)query->statement(), paramNumber, parameterMode, SQL_C_BINARY, SQL_LONGVARBINARY, len, scale, buff, SQLINTEGER(len), &cblen);
                    continue;
                }
                break;
                case VAR_DATE: {
                ...
                break;
                default:
                    query->logAndThrow("CODBCDatabase::queryBindParameters", "Unknown type of parameter "+int2string(paramNumber));
            }
            ...
            rc = SQLBindParameter((SQLHSTMT)query->statement(), paramNumber, parameterMode, paramType, sqlType, len, scale, buff, SQLINTEGER(len), cbValue);
            if (rc != 0) {
...
void SP_EXPORT spdb_odbc_presented() {}
