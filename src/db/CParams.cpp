...
//---------------------------------------------------------------------------
CParam::CParam(char *name) : CVariant() {
...
void CParam::setString(const char * value,uint32_t maxlen) {
    ...
    if (maxlen) valueLength = maxlen;
    else        valueLength = (uint32_t) strlen(value);
...
CParam& CParamList::next() {
   ...
   m_paramStreamItor++;
   if (m_paramStreamItor == m_items.end()) m_paramStreamItor = m_items.begin();
   return *currentParam;
}
//---------------------------------------------------------------------------
