...
{
    static std::map<string, VariantDataType, less<>> nameToTypeMap;

    if (nameToTypeMap.empty())
    {
        for (auto type = VariantDataType::VAR_INT;
             type <= VariantDataType::VAR_BOOL; type = VariantDataType(int(type) * 2))
        {
            auto vtype = (VariantDataType) type;
