...
void CInput_::mask(const char* m)
{
    ...
    {
        if (m[i] == '\\' || !strchr(maskControlCharacters, m[i]))
        { // background char
            if (m[i] == '\\')
            {
                i++;
        ...
        else
        {
            switch (type())
            {
                ...
                case FL_INT_INPUT:
                    ...
                    if (!strchr("0123456789XABCDEF+-", key))
                    {
                        return 0;
                    }
                    if (!pos && (key == '+' || key == '-'))
                    ...
                    if (key > '9')
                    {
                        ...
                        else if (pos > 1 && index(0) == '0' && index(1) == 'X' && (key >= 'A' && key <= 'F'))
                        {
                        }
                        else
                        {
...
CInput_::CInput_(const char* label, int layoutSize, CLayoutAlign layoutAlignment)
    : Fl_Input(0, 0, layoutSize, layoutSize, label)
