...
TEST(SPTK_ThreadManager, managedButNotStartedThreadsAreNotRunning)
{
    resetCounters();

    const auto threadManager = make_shared<ThreadManager>("Test Manager");
    const auto thread = make_shared<ThreadManagerTestThread>("thread 0");
    threadManager->manage(thread);

    EXPECT_EQ(1U, threadManager->threadCount());
    EXPECT_EQ(thread, threadManager->getNextThread());
}
