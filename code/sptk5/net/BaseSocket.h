...
public:
    ...
    /**
     * Return number of bytes available in socket
     */
    ...
    /**
     * Detach socket handle, setting it to INVALID_SOCKET.
    ...
    /**
     * Open the client socket connection by host and port
     * @param host              The host
    ...
    /**
     * Open the server socket connection on port (binds/listens)
    ...
    /**
     * Read data from the socket into memory buffer
     *
    ...
    {
        const std::scoped_lock lock(m_socketMutex);
        return writeUnlocked(buffer.data(), buffer.size(), peer);
    }
    ...
    {
        const std::scoped_lock lock(m_socketMutex);
        return writeUnlocked((const uint8_t*) buffer.data(), buffer.size(), peer);
    }

    /**
     * Report true if socket is ready for reading from it
    ...
    /**
     * Report true if socket is ready for writing to it
     * @param timeout           Write timeout
     */
    [[nodiscard]] bool readyToWrite(std::chrono::milliseconds timeout)
    {
        const std::scoped_lock lock(m_socketMutex);
...
protected:
    /**
     * Set socketFd internal (OS) handle
     */
