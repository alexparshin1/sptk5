 ...
 *
 * On Unix and Linux, the log is sent to *nix syslog daemon.
 * On Windows NT/2000+/XP the log is sent to the Event Log (Application).
 * On Windows 95/98/ME the system log isn't supported.
 * The facility method allows defining which system log is used.
 */
...
public:
    /**
     * @brief Stores or sends the log message to the .actual destination
     *
     ...
     * For Windows, the 'facilities' parameter is ignored and messages are stored
     ...
     * name is changed, the log is closed to be re-opened on the next message.
    ...
    /**
     * @brief Get log engine options.
     *
     * Retrieves the log engine output options, program name, and facilities.
