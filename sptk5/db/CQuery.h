...
namespace sptk {
   ...
   /// @brief Query lock.
   ///
   /// Used internally to provide the thread-safe
   /// query execution. Blocks the database operation if another
   /// thread is using the same database. The block will be released as
   /// soon as the thread using the database exits the database operation
   class SP_EXPORT CQueryLock {
      /// The protected database
      CDatabase *m_db;
   public:
      
      /// Constructor
      /// @param db CDatabase *, the database to protect
      CQueryLock(CDatabase *db) {
         m_db = db;
         if (m_db)
            m_db->lock();
      }
   ...
   /// depends on the CDatabase object query is connected to.
   class SP_EXPORT CQuery : public CDataSource {
   ...
   protected:
      ...
      CDatabase*        m_db;              ///< Database connection
   ...
   public:
      ...
      /// @param db CDatabase, the database to connect to, optional
      ...
      CQuery(CDatabase *db=0L, std::string sql = "", const char* createdFile=0, unsigned createdLine=0);
