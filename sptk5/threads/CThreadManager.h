...
///
/// When a thread is requested from the thread manager, it ether
...
class SP_EXPORT CThreadManager : public CSynchronized, public CThreadEvent, public CThread
{
    CSynchronizedList<CWorkerThread*>   m_terminatedThreads;    ///< Terminated threads scheduled for delete
    CSynchronizedList<CWorkerThread*>   m_threads;              ///< All threads created by this manager
    uint32_t                            m_threadLimit;          ///< Maximum number of threads in this manager
    ...
    uint32_t                            m_threadIdleSeconds;    ///< Maximum thread idle time before thread in this manager is terminated
    bool                                m_shutdown;             ///< Flag: true during manager shutdown

    /// @brief Creates a new thread and adds it to thread manager
    ///
...
protected:
    ...
    /// @brief Thread manager control thread function
    ///
...
public:
    ...
    /// @brief Constructor
    /// @param threadLimit uint32_t, Maximum number of threads in this manager
    /// @param threadIdleSeconds int32_t, Maximum period of inactivity (seconds) for thread in the manager before thread is terminated
    CThreadManager(uint32_t threadLimit=100, uint32_t threadIdleSeconds=60);

    /// @brief Destructor
    ///
    /// All worker threads are sent terminate() message,
    /// then thread manager waits while threads are destroyed
