...
enum CRefreshKind {
    LV_REFRESH_FULL,     ///< Retrieve the full dataset and replace the existing records completely
    LV_REFRESH_FAST      ///< Retrieve only the changed records and replace the changed records only
};
...
private:
    ...
    bool m_fillInProgress;                  ///< Flag to indicate the current fill() operation
    bool m_fillTerminated;                  ///< Flag to terminate the current fill() operation

protected:
    bool           m_capitalizeColumnNames; ///< Should the column headers to be capitilized?
    bool           m_multipleSelection;     ///< Is multiple selection allowed?
    int            m_horizPosition;         ///< The horizontal position - to be
    int            m_realHorizPosition;     ///< The horizontal position - existing
    int            m_maxWidth;              ///< The maximum required width for the columns
    int            m_redraw1;               ///< The minimal update pointer
    int            m_redraw2;               ///< The minimal update pointer
    uint32_t       m_top;                   ///< The scrolling position
...
protected:
    ...
    int            m_mousePressedX;         ///< The mouse pressed position
    CSelection     m_selection;             ///< The list of currently selected items
    std::string    m_searchPhrase;          ///< Type-in search buffer
    uint32_t       m_activeRow;             ///< The row where the focus is or should be
    uint32_t       m_headerHeight;          ///< The height of the header area

    CListViewRows  m_rows;                  ///< The list of rows
    CColumnList    m_columnList;            ///< The list of columns

    int            m_draggingColumn;        ///< Dragged column
    int            m_draggingStarted;       ///< Dragging column started

    Fl_Font        m_textFont;              ///< The text font type
    uchar          m_textSize;              ///< The text font size
    Fl_Color       m_textColor;             ///< The text font color

    bool           m_showStripes;           ///< Show/hide horizontal stripes. Stripes is an alternative to cell grid.
    bool           m_showGrid;              ///< Show/hide cell grid. Grid is an alternative to stripes.
    bool           m_showSelection;         ///< Show/hide selection
    bool           m_autoRowHeight;         ///< Auto row height on/off
...
public:
    ...
    void addColumn(const std::string colname,CVariantType type,uint32_t cwidth=100,bool cvisible=true) {
        m_columnList.push_back(CColumn(colname,type,cwidth,cvisible));
    }
