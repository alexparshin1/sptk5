...
typedef CLayoutClient* (*createControlCallback)(CXmlNode *node);
...
enum CLayoutGrowMode {
    LGM_NONE,                 ///< Don't allow layout to grow
    LGM_HORIZONTAL_GROW = 1,  ///< Allow horizontal grow only
    LGM_VERTICAL_GROW = 2,    ///< Allow vertical grow only
    LGM_AUTO_GROW = 3         ///< Allow both horizontal and vertical grow
};
...
protected:
    int               m_layoutSpacing;   ///< An extra space between widgets
    CLayoutGrowMode   m_layoutGrowMode;  ///< Layout grow mode, @see CLayoutGrowMode
    Fl_Group         *m_group;           ///< Attached group to manage
    CFrame           *m_frame;           ///< The group frame, including optional background image
    bool              m_frameDrawBackground; ///< Should we draw background
    bool              m_noXml;           ///< If true - do not read or write any XML

    static std::map<std::string,Fl_Boxtype> m_boxTypeNames; ///< A map of box type names and corresponding constants
    static createControlCallbackMap controlCreator;         ///< A map of control types and callbacks that create them
...
public:
    ...
    virtual void loadLayout(const CXmlNode* node,CLayoutXMLmode xmlMode) THROWS_EXCEPTIONS;
