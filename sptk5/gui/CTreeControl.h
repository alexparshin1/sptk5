...
class CTreeItem : public CGroup
{
    ...
    int             m_labelHeight;  ///< The height of the label computed by fl_measure
    bool            m_selected;     ///< Is the item selected (controlled by parent tree control)
    Fl_Color        m_itemColor[2]; ///< Item colors (text,bg) before selection

protected:
    CLayoutClient  *m_body;         ///< The widget that represents the item without a child
    const Fl_Image *m_openedImage;  ///< The image for the open state
    const Fl_Image *m_closedImage;  ///< The image for the closed state
    bool            m_opened;       ///< The item's state
...
public:
    ...
    static const Fl_Image* treeClosed;            ///< Default image of the closed tree
    static const Fl_Image* folderOpened;          ///< Default image of the opened floder
    static const Fl_Image* folderClosed;          ///< Default image of the closed floder
    static const Fl_Image* document;              ///< Default image of the document
...
private:
    ...
    CTreeItem*        m_root;          ///< The root item
protected:
    bool              m_tabPressed;    ///< Internal flag to temporarely block tree items focus acceptance
    CTreeItemCreator  m_itemCreator;   ///< The tree item creator creates a tree item' body, @see CTreeItemCreator
